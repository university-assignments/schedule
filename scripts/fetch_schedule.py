...
Скрипт делает ровно три вещи и запускается вручную:
...
def fetch_token() -> str:
	...
	SUSU_TOKEN берётся как есть: JWT живёт около суток, на ручной запуск его хватает.
	Без него логин с паролем меняются на свежий токен через SUSU_AUTH_URL.
	"""
