...
Скрипт делает ровно три вещи и запускается по расписанию из GitHub Actions:
...
Токен живёт около суток, поэтому для cron он не годится: положите в секреты логин и
пароль и задайте SUSU_AUTH_URL — тогда свежий токен берётся на каждом прогоне.
"""
...
def fetch_token() -> str:
	...
	Сам JWT живёт около суток, поэтому класть его в секреты репозитория бессмысленно:
	cron сломается на второй день. Рабочая схема — логин и пароль в секретах и обмен их
	на свежий токен каждый прогон; SUSU_TOKEN остаётся для ручных запусков.
	"""
	...
	if not (login and password and auth_url):
		sys.exit(
			"Нет доступа к API: задайте SUSU_TOKEN (разовый запуск) либо "
			"SUSU_LOGIN + SUSU_PASSWORD + SUSU_AUTH_URL (cron)."
		)
...
def run(dry_run: bool) -> int:
	...
	# Код возврата всегда нулевой, а «есть ли что коммитить», workflow спрашивает у
	# git status. Отдельный код для «изменений нет» неотличим от единицы, которой
	# завершается упавший запрос, — и молчаливо сломанный cron выглядел бы как тишина
	# в расписании.
	return 0
